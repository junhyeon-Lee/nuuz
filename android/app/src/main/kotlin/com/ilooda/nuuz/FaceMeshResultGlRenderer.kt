...
//            )
       ...
       //     drawCircle(result.multiFaceLandmarks()[i].landmarkList[0].x,result.multiFaceLandmarks()[i].landmarkList[0].y, floatArrayOf(1f, 0.2f, 0.2f, 1f))
//            drawLandmarks(
//                result.multiFaceLandmarks()[i].landmarkList,
...
//            }
    ...
    private val NUM_SEGMENTS = 3
    val LANDMARK_RADIUS = 0.008f
    fun drawCircle(x: Float, y: Float, colorArray: FloatArray?) {
        ...
        for (i in 1 until vertexCount) {
            val angle: Float =
                2.0f * i * Math.PI.toFloat() / NUM_SEGMENTS
            val currentIndex = 3 * i
            vertices[currentIndex] =
                x + (LANDMARK_RADIUS * Math.cos(
                    angle.toDouble()
                )).toFloat()
